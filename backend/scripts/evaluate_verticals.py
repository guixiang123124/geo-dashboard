#!/usr/bin/env python3
"""
Evaluate industry vertical brands across prompts using Gemini 2.0 Flash.
Supports 6 verticals: health, fintech, travel, realestate, education, food.

Usage:
    python backend/scripts/evaluate_verticals.py --vertical health --seed-only
    python backend/scripts/evaluate_verticals.py --vertical fintech
    python backend/scripts/evaluate_verticals.py --seed-all
"""

import json, time, uuid, sys, os, re, argparse
...
VERTICALS = {
    "health": {
        ...
        "brands_file": "health_brands.json",
        "prompts_file": "health_prompts.json",
    },
    "fintech": {
        "category": "Fintech & Financial Services",
        "brands_file": "fintech_brands.json",
        "prompts_file": "fintech_prompts.json",
    },
    "travel": {
        "category": "Travel & Hospitality",
        "brands_file": "travel_brands.json",
        "prompts_file": "travel_prompts.json",
    },
    "realestate": {
        "category": "Real Estate & Home",
        "brands_file": "realestate_brands.json",
        "prompts_file": "realestate_prompts.json",
    },
    "education": {
        "category": "Education & EdTech",
        "brands_file": "education_brands.json",
        "prompts_file": "education_prompts.json",
    },
    "food": {
        "category": "Food & Beverage",
        "brands_file": "food_brands.json",
        "prompts_file": "food_prompts.json",
    },
}
...
# ── Seed brands & prompts ──────────────────────────────────────────────────
def seed_vertical(cur, vertical_key):
    """Insert brands and prompts for a vertical."""
    cfg = VERTICALS[vertical_key]
    category = cfg["category"]
    brands = load_json(SCRIPT_DIR / cfg["brands_file"])
    prompts = load_json(SCRIPT_DIR / cfg["prompts_file"])
    ...
    print(f"[{vertical_key}] Seeded {inserted_brands} new brands, {inserted_prompts} new prompts (category: {category})")
...
# ── Run evaluation ─────────────────────────────────────────────────────────
def run_evaluation(cur, vertical_key, brands, prompts, brand_ids, prompt_ids):
    cfg = VERTICALS[vertical_key]
    ...
    """, (run_id, WORKSPACE_ID, f"{cfg['category']} Evaluation - Gemini Flash", json.dumps([MODEL]), len(prompts)))
...
def compute_score_cards(cur, run_id, brand_ids):
    ...
    for bname, bid in brand_ids.items():
        ...
        intent_coverage = intent_cats / 10.0
...
# ── Main ────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Evaluate industry verticals")
    parser.add_argument("--vertical", choices=list(VERTICALS.keys()), help="Vertical to evaluate")
    parser.add_argument("--seed-only", action="store_true", help="Only seed brands and prompts, skip evaluation")
    parser.add_argument("--seed-all", action="store_true", help="Seed all 6 verticals")
    args = parser.parse_args()

    if not args.vertical and not args.seed_all:
        parser.error("Either --vertical or --seed-all is required")
    ...
    if args.seed_all:
        for vkey in VERTICALS:
            seed_vertical(cur, vkey)
        pg.commit()
        print(f"\n✅ All 6 verticals seeded to Railway DB")

        # Summary
        for vkey, cfg in VERTICALS.items():
            cur.execute("SELECT COUNT(*) FROM brands WHERE category = %s", (cfg["category"],))
            cnt = cur.fetchone()[0]
            print(f"   {cfg['category']}: {cnt} brands")
    ...
    # Single vertical
    brands, prompts, brand_ids, prompt_ids = seed_vertical(cur, args.vertical)
    pg.commit()
    print(f"✅ {args.vertical} brands and prompts seeded to Railway DB")
    ...
    # Run evaluation
    run_id = run_evaluation(cur, args.vertical, brands, prompts, brand_ids, prompt_ids)
    pg.commit()
    ...
    """, (run_id,))
    print(f"\nTop 10 {args.vertical} brands by composite score:")
