...
const NAV_ITEMS = [
  {
    label: 'Product',
    items: [
      { name: 'AI Brand Diagnosis', desc: 'Instant AI visibility score', href: '/audit', icon: Crosshair },
      { name: 'Dashboard', desc: 'Monitor your brand metrics', href: '/dashboard', icon: LayoutDashboard },
      { name: 'Competitive Analysis', desc: 'Benchmark vs competitors', href: '/compete', icon: GitCompare },
      { name: 'Prompt Research', desc: 'Discover AI search prompts', href: '/prompts', icon: Search },
      { name: 'Trend Tracking', desc: 'Track visibility over time', href: '/trends', icon: TrendingUp },
    ],
  },
  {
    label: 'Solutions',
    items: [
      { name: 'For E-commerce Brands', desc: 'Boost product visibility in AI', href: '#diagnosis', icon: ShoppingCart },
      { name: 'For Agencies', desc: 'White-label GEO tools', href: '#pricing', icon: Users },
      { name: 'For Enterprise', desc: 'Custom integrations & support', href: '#pricing', icon: Building2 },
    ],
  },
  { label: 'Pricing', href: '/pricing' },
  {
    label: 'Learn',
    items: [
      { name: 'GEO Learning Center', desc: 'Master generative engine optimization', href: '/learn', icon: GraduationCap },
      { name: 'Optimization Guide', desc: 'Step-by-step playbook', href: '/optimize', icon: BookMarked },
      { name: 'Blog', desc: 'Coming soon', href: '#', icon: FileText },
    ],
  },
] as const;
...
function NavDropdown({ item, open, onToggle, onClose }: { item: NavItem & { items: readonly { name: string; desc: string; href: string; icon: React.ElementType }[] }; open: boolean; onToggle: () => void; onClose: () => void }) {
  const ref = useRef<HTMLDivElement>(null);
  ...
  return (
    ...
    >
      <button
        onClick={onToggle}
        className="flex items-center gap-1 text-sm text-slate-600 hover:text-violet-600 transition"
      >
        {item.label}
      ...
      {open && (
        <div className="absolute top-full left-1/2 -translate-x-1/2 pt-2">
          <div className="bg-white rounded-xl border border-slate-200 shadow-lg shadow-slate-200/50 p-2 min-w-[280px]">
            {item.items.map((sub) => (
              <Link
                key={sub.name}
                href={sub.href}
                onClick={onClose}
                className="flex items-start gap-3 px-3 py-2.5 rounded-lg hover:bg-violet-50 transition group"
              >
                <sub.icon className="w-5 h-5 text-slate-400 group-hover:text-violet-600 mt-0.5 shrink-0" />
                <div>
    ...
    <nav ref={navRef} className="fixed top-0 w-full bg-white/80 backdrop-blur-md border-b border-slate-200 z-50">
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6">
          {NAV_ITEMS.map((item) =>
            'items' in item ? (
              ...
              <Link key={item.label} href={item.href} className="text-sm text-slate-600 hover:text-violet-600 transition">
                {item.label}
              </Link>
            )
          )}
        ...
        <div className="flex items-center gap-3">
          <Link href="/auth/login" className="hidden md:block text-sm text-slate-600 hover:text-violet-600 transition">
            Sign In
          </Link>
          <Link href="/audit">
            <Button size="sm" className="bg-violet-600 hover:bg-violet-700">Free Brand Audit</Button>
          </Link>
          {/* Mobile hamburger */}
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="md:hidden bg-white border-t border-slate-100 px-6 py-4 max-h-[calc(100vh-4rem)] overflow-y-auto">
          {NAV_ITEMS.map((item) =>
            'items' in item ? (
          ...
          <div className="pt-3 space-y-2">
            <Link href="/auth/login" onClick={() => setMobileOpen(false)} className="block py-2 text-sm text-slate-600">
              Sign In
            </Link>
            <Link href="/dashboard" onClick={() => setMobileOpen(false)} className="block py-2 text-sm text-slate-600">
              Dashboard
            </Link>
          </div>
        </div>
